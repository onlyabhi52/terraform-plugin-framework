...
func TestValueAs(t *testing.T) {
	...
	tests := map[string]testCase{
		...
		"incompatible-type": {
			...
			expectedDiags: diag.Diagnostics{reflect.NewDiagIntoIncompatibleType(
				tftypes.NewAttributePath(),
				tftypes.NewValue(tftypes.String, "hello"),
				goreflect.TypeOf(int64(0)),
				fmt.Errorf("can't unmarshal %s into %T, expected *big.Float", tftypes.String, big.NewFloat(0)),
			)},
		},
		"different-type": {
			val:    types.String{Value: "hello"},
			target: &testtypes.String{},
			expectedDiags: diag.Diagnostics{reflect.NewDiagNewAttributeValueIntoWrongType(
				tftypes.NewAttributePath(),
				goreflect.TypeOf(types.String{Value: "hello"}),
				goreflect.TypeOf(testtypes.String{}),
				types.StringType,
			)},
		},
	}
